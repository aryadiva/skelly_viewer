...
class FreeMoCapDataLoader:
    ...
    # addition
    def find_angles_data(self) -> Path:
        angles_path_list = [path.name for path in self.find_output_data_folder_path().glob("*.csv")]

        if 'angles.csv' in angles_path_list:
            return self.find_output_data_folder_path() / 'angles.csv'

        raise Exception(f"Could not find angles.csv file in path {str(self.find_output_data_folder_path())}")
